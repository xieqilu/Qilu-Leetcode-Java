...
public class Solution {
    public int compareVersion(String version1, String version2) {
        ...
        while(i!=v1.length){
            if(Integer.parseInt(v1[i])!=0) return 1; //if any part is not 0, then v1>v2
            i++;
        }
        //if j==v2.length, the loop won't be executed.
        while(j!=v2.length){
            if(Integer.parseInt(v2[j])!=0) return -1; //if any part is not 0, then v2>v1
            j++;
        }
        return 0;
    }
...
public class Solution {
    public int compareVersion(String version1, String version2){
        ...
        while(i<v1.length||i<v2.length){
            //use 0 to represent missing part of shorter version
