...
public class Solution {
    public int[] twoSum(int[] numbers, int target) {
        for(int i =0;i<numbers.length;i++){
            ...
            if(result!=-1)
                return new int[]{i+1, result+1};//note returned index is not zero-based
        }
...
//Solution2: Two Pointers, Time: O(n) Space:O(1)
public class Solution {
    public int[] twoSum(int[] numbers, int target) {
        ...
        while(p1<p2){ //if p1==p2, search should be over
            ...
            if(sum>target)
                ...
                return new int[]{p1+1,p2+1}; //note returned index is not zero-based
        }
        throw new IllegalArgumentException("No Two Sum Solution!");
    }
}
