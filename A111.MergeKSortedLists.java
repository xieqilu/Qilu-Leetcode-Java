...
public class Solution {
    public ListNode mergeKLists(List<ListNode> lists) {
        ...
        //declare a generic PriorityQueue with size of k, and use a self-defined
        //comparator to specify how the element will be compared.
        ...
        //add all the head nodes of lists to Priority Queue
        ...
        ListNode p = head; //p serves as a pointer to construct the new sorted list
        
        while(pq.size()!=0){ //while PriorityQueue is not empty
            ListNode temp = pq.poll(); //poll() is like remove()
            p.next = temp;
            p = p.next;
        ...
        return head.next; //Note: head is a useless head node, we need to return head.next!
    }
}
